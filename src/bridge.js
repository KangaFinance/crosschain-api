...
module.exports.Bridge = async function(trx, node, gasLimit, exchangeMode, networkType, ActionType, contractAbiJson, contractAddress, contractManagerAbiJson, contractManagerAddress, wallet, amount) {
  return await bridge(trx, node, gasLimit, exchangeMode, networkType, ActionType, contractAbiJson, contractAddress, contractManagerAbiJson, contractManagerAddress, wallet, amount);
}
...
async function deposit(node, gasLimit, abiJson,  wallet, amountInWei) {
  ...
  const contract = new web3.eth.Contract(contractJson.abi, process.env.HMY_DEPOSIT_CONTRACT_2);
...
async function burnTxn(node, gasLimit, abiJson, contractManagerAddress, wallet, amountInWei) {
  ...
  let transaction = await busdContract.methods
    .burnToken(amountInWei, account.address)
    .send({
      from: account.address,
      gas: gasLimit,
      gasPrice: new BN(await web3.eth.getGasPrice()).mul(new BN(1)),
...
async function bridge(trx, node, gasLimit, exchangeMode, networkType, ActionType, contractAbiJson, contractAddress, contractManagerAbiJson, contractManagerAddress, wallet, amount) {
  try {
    ...
    switch (trx) {
      ...
      console.log("Trx:", "Burn it!") 
        ...
        break;
      default:
        return { trx: "swap", success: false, error_message: "Wrong transaction value", error_body: "Only possible values are 0 or 1"}
    }
